...
DEL /F dist\%CRATE_NAME_SNAKE_CASE%_bg.wasm
...
wasm-bindgen "%TARGET%\wasm32-unknown-unknown\%BUILD%\%TARGET_NAME%" --out-dir "dist" --no-modules --no-typescript

IF %FAST% == 0 (
  echo Optimizing wasm...
  @REM to get wasm-opt:  apt/brew/dnf install binaryen
  @REM add -g to get debug symbols :
  wasm-opt "dist\%CRATE_NAME%_bg.wasm" -O2 -o "dist\%CRATE_NAME%_bg.wasm"
)
